...
class GetTrainingPlansByOwnerIdUseCase(private val trainingPlanRepository: TrainingPlanRepository) {
    suspend fun execute(): Result<List<TrainingPlanDto>> {
        val uid = Firebase.auth.currentUser?.uid
            ?: return Result.Failure(NullPointerException("UID is null"))
        return trainingPlanRepository.getTrainingPlansByOwnerId(uid)
    }
}